...
{
	private static volatile T s_tInstance = null;
		...
		get
		{
			lock(s_lockobj)
			{
				if(s_tInstance == null) // instance not yet created
				{
					//create the instance.
					Type t = typeof(T);
//					DebugUtils.Assert(!s_lockobj.IsInstanceBeingCreated,  "Invalid recursive call to " + t.Name + ".Instance" );
					...
					ISingleton cbSingleCreated = s_tInstance as ISingleton;
				...
				if (_instance == null)
				{
					...
					*/
					...
					if (_instance == null)
					{
                        ...
                        //DebugUtils.Log("[Singleton] Using instance already created: " +
                        //          _instance.gameObject.name);
					}

					ISingleton cbSingleCreated = _instance as ISingleton;
					if(cbSingleCreated != null)
						cbSingleCreated.OnInstanceCreated();
				}
